...
\begin{document}
    ...
    While in some circuits `instant feedback' is useful~\cite{riedel2004cyclic,mendler2012constructive}, in other cases it can be troublesome.
    ...
    \noindent
    The complete set of axioms for closed circuits is shown in \cref{fig:closed-circuit-axioms}.
    That this is the `correct' set is justified by the following theorem:
    ...
    Independent of a signature, all circuits will include the generators for fork, join, stub and the disconnected wire.
    ...
    Finally, we must add any axioms for equivalences between gates.

    \begin{definition}
        We say that a set of equations \(\mathcal{E}\) is \emph{combinationally complete} for an interpretation \(\interpretation\) if for all combinational circuits \(\iltikzfig{circuits/components/circuits/f-comb}\) and \(\iltikzfig{circuits/components/circuits/g-comb}\) containing at least one gate generator, if \(\circuittostream[\iltikzfig{circuits/components/circuits/f-comb}]{\interpretation} = \circuittostream[\iltikzfig{circuits/components/circuits/g-comb}]{\interpretation}\) then there exists a sequence of axioms in \(\mathcal{E}\) such that \(\circuittostream[\iltikzfig{circuits/components/circuits/f-comb}]{\interpretation} \eqaxioms{\mathcal{E}}\circuittostream[\iltikzfig{circuits/components/circuits/g-comb}]{\interpretation}\).
    \end{definition}
    ...
    \begin{figure}[p]
        ...
        \begin{align*}
            ...
            &
            \iltikzfig{circuits/axioms/belnap/and-commutative-lhs}
            &\eqaxioms{\mathcal{R}}
            \iltikzfig{circuits/axioms/belnap/and-commutative-rhs}
            &
            \iltikzfig{circuits/axioms/belnap/or-commutative-lhs}
            &\eqaxioms{\mathcal{R}}
            \iltikzfig{circuits/axioms/belnap/or-commutative-rhs}
        \end{align*}
        \caption{A set of axioms \(\mathcal{R}\) which is combinationally complete for \(\interpretation_\star\).}
